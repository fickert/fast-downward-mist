...
class VarValueRenaming(object):
    ...
    def apply_to_task(self, task):
        ...
        self.apply_to_goals(task.goal.pairs, task.online_goals.subgoals, task.online_goals.subgoals_availability)
    ...
    def apply_to_goals(self, goals, online_goals, online_goals_availability):
        # This may propagate Impossible up.
        self.convert_pairs(goals)
        # convert online goals, remove empty online goals
        assert(len(online_goals) == len(online_goals_availability))
        for i in range(len(online_goals) - 1, -1, -1):
            self.convert_pairs(online_goals[i])
            if not online_goals[i]:
                del online_goals[i]
                del online_goals_availability[i]
        if not goals and not online_goals:
