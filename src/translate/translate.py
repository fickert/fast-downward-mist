...
def translate_task(strips_to_sas, ranges, translation_key,
                   ...
                   init, goals, online_goals, online_goals_availability,
                   actions, axioms, metric, execution_time, implied_facts):
    ...
    goal_dict_list = translate_strips_conditions(goals, strips_to_sas, ranges,
                                                 mutex_dict, mutex_ranges)
    online_goal_dict_lists = []
    for subgoal in online_goals:
        online_goal_dict_lists.append(translate_strips_conditions(subgoal, strips_to_sas, ranges,
                                                                  mutex_dict, mutex_ranges))
    if goal_dict_list is None or any(online_goal_dict_list is None for online_goal_dict_list in online_goal_dict_lists):
        # "None" is a signal that the goal is unreachable because it
        # violates a mutex.
        return unsolvable_sas_task("Goal violates a mutex")

    assert len(goal_dict_list) == 1 and all(len(online_goal_dict_list) == 1 for online_goal_dict_list in online_goal_dict_lists), "Negative goal not supported"
    ...
    online_goal_pairs_list = [list(online_goal_dict_list[0].items()) for online_goal_dict_list in online_goal_dict_lists]

    if not goal_pairs and all(not online_goal_pairs for online_goal_pairs in online_goal_pairs_list):
        return solvable_sas_task("Empty goal")

    goal = sas_tasks.SASGoal(goal_pairs)
    online_goal = sas_tasks.SASOnlineGoals(online_goal_pairs_list, online_goals_availability)
    ...
    return sas_tasks.SASTask(variables, mutexes, init, goal, online_goal,
...
def main():
    ...
    with timers.timing("Writing output"):
        with open("output.sas", "w") as output_file:
            sas_task.output(output_file)  # ~~! here make output for all
    print("Done! %s" % timer)
