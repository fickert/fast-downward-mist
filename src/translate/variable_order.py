...
class CausalGraph(object):
    ...
    def __init__(self, sas_task):
        ...
        for subgoal in sas_task.online_goals.subgoals:
            self.goal_map.update(subgoal)
    ...
    def calculate_important_vars(self, sas_task):
        ...
        self.calculate_important_vars_pairs(sas_task.goal.pairs, necessary)

        for subgoal in sas_task.online_goals.subgoals:
            self.calculate_important_vars_pairs(subgoal, necessary)

        return necessary

    def calculate_important_vars_pairs(self, pairs, necessary):
        for var, _ in pairs:
...
class VariableOrder(object):
    ...
    def apply_to_task(self, sas_task):
        ...
        self._apply_to_online_goals(sas_task.online_goals)
    ...
    def _apply_to_online_goals(self, online_goals):
        online_goals.subgoals = [sorted((self.new_var[var], val)
                                        for var, val in subgoal
                                        if var in self.new_var) for subgoal in online_goals.subgoals]
...
def find_and_apply_variable_order(sas_task, reorder_vars=True,
    ...
    for var, val in sas_task.goal.pairs:
        all_vars.append((var, val))
    for online_goal in sas_task.online_goals.subgoals:
        for var, val in online_goal:
            all_vars.append((var, val))
    ...
    if reorder_vars or filter_unimportant_vars:
        ...
        if filter_unimportant_vars:
            necessary = cg.calculate_important_vars(sas_task)
