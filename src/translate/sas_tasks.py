...
class SASTask:
    ...
    def __init__(self, variables, mutexes, init, goal, online_goals,
                 operators, axioms, metric, execution_time):
        self.variables = variables
        self.mutexes = mutexes
        self.init = init
        self.goal = goal
        self.online_goals = online_goals
    ...
    def validate(self):
        ...
        assert self.execution_time is None or self.execution_time >= 1
    ...
    def output(self, stream):
        ...
        self.goal.output(stream) # ~~! invoke output on task add both goal and onlinegoals objects
        self.online_goals.output(stream) # ~~! invoke output on task add both goal and onlinegoals objects
        print("begin_execution_time", file=stream)
        print(self.execution_time or "0", file=stream)
        print("end_execution_time", file=stream)
...
class SASOnlineGoals:
    def __init__(self, subgoals, subgoals_availability):
        self.subgoals = [sorted(subgoal) for subgoal in subgoals]
        self.subgoals_availability = subgoals_availability

    def validate(self, variables):
        """Assert that the goal is nonempty and a valid condition."""
        for subgoal in self.subgoals:
            assert subgoal
            variables.validate_condition(subgoal)

    def dump(self):
        for subgoal in self.subgoals:
            for var, val in subgoal:
                print("v%d: %d" % (var, val))

    def output(self, stream):
        subgoal_counter = 0
        print("begin_onlinegoals", file=stream)
        print(len(self.subgoals), file=stream)
        for pairs in self.subgoals:
            print("begin_onlinegoal", file=stream)
            print(len(pairs), file=stream)
            for var, val in pairs:
                print(var, val, file=stream)
            print(self.subgoals_availability[subgoal_counter], file=stream)
            subgoal_counter += 1
            print("end_onlinegoal", file=stream)
        print("end_onlinegoals", file=stream)
