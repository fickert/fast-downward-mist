...
def parse_task(domain_pddl, task_pddl):
    ...
    task_name, task_domain_name, task_requirements, objects, init, goal, online_goals, online_goals_availability, execution_time, use_metric \
    ...
    return pddl.Task(
        ...
        predicates, functions, init, goal, online_goals, online_goals_availability, actions, axioms, execution_time, use_metric)  # ~~! here create task
...
def parse_task_pddl(task_pddl, type_dict, predicate_dict):
    ...
    subgoals = []
    subgoals_availability = []
    execution_time = None
    use_metric = False
    for entry in iterator:
        if entry[0] == ":onlinegoals":
            subgoals, subgoals_availability = parse_online_goals(entry, type_dict, predicate_dict, False)
        if entry[0] == ":metric":
            if entry[1] == "minimize" and entry[2][0] == "total-cost":
                use_metric = True
            else:
                assert False, "Unknown metric."
        if entry[0] == ":execution_time":
            assert entry[1].isdigit()
            execution_time = int(entry[1])
    yield subgoals
    yield subgoals_availability
