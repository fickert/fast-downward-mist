...
RelaxedSumEvaluator::RelaxedSumEvaluator(const Options &opts)
    : CombiningEvaluator(opts.get_list<shared_ptr<Evaluator>>("evals")) {
}

RelaxedSumEvaluator::RelaxedSumEvaluator(const vector<shared_ptr<Evaluator>> &evals)
    : CombiningEvaluator(evals) {
}

RelaxedSumEvaluator::~RelaxedSumEvaluator() {
}

int RelaxedSumEvaluator::combine_values(const vector<int> &values) {
    int result = 0;
    for (int value : values)
        result += value;
    return result;
