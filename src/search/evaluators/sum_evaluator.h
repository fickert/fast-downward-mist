...
class RelaxedSumEvaluator : public combining_evaluator::CombiningEvaluator {
protected:
	virtual int combine_values(const std::vector<int> &values) override;
public:
	explicit RelaxedSumEvaluator(const options::Options &opts);
	explicit RelaxedSumEvaluator(const std::vector<std::shared_ptr<Evaluator>> &evals);
	virtual ~RelaxedSumEvaluator() override;
};
}
