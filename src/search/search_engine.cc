...
SearchEngine::SearchEngine(const Options &opts, std::shared_ptr<StateRegistry> state_registry, std::shared_ptr<AbstractTask> task, bool use_default_successor_generator)
    : status(IN_PROGRESS),
      solution_found(false),
      task(task),
      task_proxy(*task),
      state_registry(state_registry),
      successor_generator(get_successor_generator(use_default_successor_generator ? TaskProxy(*tasks::g_root_task) : task_proxy)),
      search_space(*state_registry),
      cost_type(static_cast<OperatorCost>(opts.get_enum("cost_type"))),
      is_unit_cost(task_properties::is_unit_cost(task_proxy)),
      max_time(opts.get<double>("max_time")) {
