...
EagerSearch::EagerSearch(const Options &opts)
    : SearchEngine(opts),
      ...
      initial_state_id(StateID::no_state) {
...
EagerSearch::EagerSearch(const options::Options& opts, std::shared_ptr<StateRegistry> state_registry, std::shared_ptr<AbstractTask> task, StateID initial_state_id, bool use_default_successor_generator)
	: SearchEngine(opts, state_registry, task, use_default_successor_generator),
	  reopen_closed_nodes(opts.get<bool>("reopen_closed")),
...
void EagerSearch::initialize() {
    ...
    */
    ...
    const GlobalState &initial_state = initial_state_id != StateID::no_state ?
		state_registry->lookup_state(initial_state_id) : state_registry->get_initial_state();
