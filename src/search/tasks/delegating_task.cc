...
int DelegatingTask::get_num_online_goals() const {
	return parent->get_num_online_goals();
}

std::pair<std::vector<FactPair>, int> DelegatingTask::get_online_goal(int index) const {
	return parent->get_online_goal(index);
}

int DelegatingTask::get_execution_time() const {
	return parent->get_execution_time();
}
