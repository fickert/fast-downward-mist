...
class RootTask : public AbstractTask {
	...
	vector<pair<vector<FactPair>, int>> online_goals;
	int execution_time;
...
public:
	...
	virtual int get_num_online_goals() const override;
	virtual std::pair<std::vector<FactPair>, int> get_online_goal(int index) const override;

	virtual int get_execution_time() const override;
...
void check_magic(istream &in, const string &magic) {
    string word;
    in >> word;
    bool online = word.find("online") != string::npos;
    if (word != magic && !online) {
...
vector<pair<vector<FactPair>, int>> read_online_goals(istream &in) {
	check_magic(in, "begin_onlinegoals");
	int count;
	in >> count;
	vector<pair<vector<FactPair>, int>> online_goals;
	online_goals.reserve(count);
	for (auto i = 0; i < count; ++i) {
		check_magic(in, "begin_onlinegoal");
		auto goal_facts = read_facts(in);
		int expansions;
		in >> expansions;
		online_goals.emplace_back(std::move(goal_facts), expansions);
		check_magic(in, "end_onlinegoal");
	}
	check_magic(in, "end_onlinegoals");
	return online_goals;
}

int read_execution_time(istream &in) {
	check_magic(in, "begin_execution_time");
	int execution_time;
	in >> execution_time;
	check_magic(in, "end_execution_time");
...
RootTask::RootTask(std::istream &in) {
	...
	online_goals = read_online_goals(in);
	for (const auto &online_goal : online_goals)
		check_facts(online_goal.first, variables);
	execution_time = read_execution_time(in);
...
int RootTask::get_num_online_goals() const {
	return online_goals.size();
}

std::pair<std::vector<FactPair>, int> RootTask::get_online_goal(int index) const {
	assert(utils::in_bounds(index, online_goals));
	return online_goals[index];
}

int RootTask::get_execution_time() const {
	return execution_time;
}
